...
export class MongoDbProtocol extends DBProtocol<ProtocolConfigParams> {
  ...
  public validateConfiguration () : void {
    ...
    if (this.protocolConfiguration.dbConnectionString) {
...
const classInstance = new MongoDbProtocol(
  {
    databaseName: "cu",
...
// eslint-disable-next-line max-lines-per-function
classInstance.initialize().then(async () => {
  // await classInstance.insert("wallet", {
  //   "name": "Johnathan",
  //   "dollarBillsInVirtualWallet": [5, 5, 5, 50, 10],
  // });

  // await classInstance.insert("wallet", {
  //   "name": "Rob",
  //   "dollarBillsInVirtualWallet": [5, 10, 50, 100, 2],
  // });

  // await classInstance.insert("wallet", {
  //   "name": "Mary",
  //   "dollarBillsInVirtualWallet": [5, 5, 5, 20, 50],
  // });

  const result = await classInstance.find("wallet", {
    query: { dollarBillsInVirtualWallet: {
      contains_one_of: [2, 10],
    } },
  });
